...
class CMultiFileUpload extends CInputWidget
{
	...
	{	
		...
		$album = $this->model->findByPk($this->model->getPrimaryKey());

		if($album){
			$images=$album->contentImages;
					foreach ($images as $key => $image) {
					?>

					<div id="image-<?php echo $image->id; ?>" style="float:left;">
					<div style="height:20px;width:190px;background-color:black; color:white; padding-left:10px;" >
					<a onClick="ObrisiSliku(<?php echo $image->id; ?>)" >Obrisi sliku </a>
					</div>
					<img src="/clientpub/images/content-album/thumb/<?php echo $image->image_path;?>" class="multifile_pic_delete"  onClick="window.open('/clientpub/images/content-album/normal/<?php echo $image->image_path;?>')" />
					</div>
