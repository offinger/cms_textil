...
class DstImageValidator extends CValidator {
    ...
    protected function validateAttribute($object, $attribute) {
        ...
        if($fileField) {
            ...
            if(!$imageParams) {
                $this->addError($object,$attribute,Yii::t('yii','{attribute} nije validna fotografija.'));    
            } else {
                if(!$this->areDimensionsCorrect($imageParams['width'], $imageParams['height'])) {
                    $this->addError($object,$attribute,Yii::t('yii','{attribute} nije u dozvoljenim dimenzijama.'));     
                }
            }
        }
    }  
