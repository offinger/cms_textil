...
class Modeli extends CActiveRecord{
	...
	public function rules() {
		return array(
			...
			array('ime, sifra, brend_id, foto, kategorija, kolekcija_id, status', 'required'),
			array('kolekcija_id, status, views', 'numerical', 'integerOnly'=>true),
			array('ime, sifra, foto, kategorija', 'length', 'max'=>255),
	...
	protected function saveFormPostprocess($data) {
        ...
        if($state == DstImageField::STATE_CHOSEN) {
            ...
            $image_path->saveAs('/home/tgroupco/public_html/clientpub/images/content-model/' . $image_path, false);
            $image = Yii::app()->image->load('/home/tgroupco/public_html/clientpub/images/content-model/' . $image_path);
